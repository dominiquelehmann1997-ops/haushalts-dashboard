...
- [x] **Step 1: Failing test ergänzen**
...
- [x] **Step 5: Route auf den Service reduzieren**
...
- [x] **Step 6: Voller Testlauf + Commit**
...
- [x] **Step 2: package.json-Script ergänzen**
...
- [x] **Step 3: Smoke-Run (manuell, im Worktree mit gültigem Token)**
...
- [x] **Step 4: `tablet-start.sh` — Sync vor Plan**
...
- [x] **Step 5: `tablet-sync.sh` für den stündlichen Cron**
...
- [x] **Step 6: Termux-Cron einhängen (Dokumentation im Script-Kopf-Kommentar + manuell am Tablet)**
...
- [x] **Step 1: Failing test (busy blockt Person → Aufgabe geht an die andere)**
...
- [x] **Step 3: `planDueTasks` — dayLoad ableiten & durchreichen**
...
- [x] **Step 5: `planToday.ts` — busy + forecast injizieren**
...
- [x] **Step 2: SyncButton-Komponente**
...
- [x] **Step 3: Tablet — Button in Header-Topbar**
...
- [x] **Step 4: Handy — Button in PageHeader `right`-Slot**
...
- [x] **Step 6: Manueller Klick-Test (Dev-Server im Worktree)**
...
- [x] **Voller Testlauf:** `npm test` → alle grün (237 + neue).
- [x] **Typecheck:** `npm run typecheck` → sauber.
- [x] **Final-Review** (Cross-Cutting über den ganzen Branch) via `superpowers:requesting-code-review` — Fokus: Kapazitäts-Mathematik, Sync-Fehlerpfade, keine Prod-DB-Berührung.
- [x] **Finishing:** `superpowers:finishing-a-development-branch` → Merge nach `main`, dann am Tablet `git pull`, `npm run build`, Neustart (`scripts/tablet-start.sh`) + einmalig Termux-Cron einhängen (Task 5 Step 6).
